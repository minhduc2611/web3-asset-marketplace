...
const GraphCanvas = forwardRef<GraphCanvasRef, GraphCanvasProps>(
  (
    ...
    }>({ open: false, searchTerm: "", nodeId: "", systemInstruction: "" });
    ...
    // Handle confirmed Google search with system instruction
    const handleConfirmedGoogleSearch = (systemInstruction: string) => {
      const { searchTerm, nodeId } = googleSearchConfirmDialog;

      // Close confirmation modal and open results modal
      setGoogleSearchConfirmDialog((prev) => ({ ...prev, open: false }));
      setGoogleSearchDialog((prev) => ({
        ...prev,
        open: true,
        searchTerm,
    ...
    useEffect(() => {
      ...
      cyRef.current.on("touchend", "node", (event) => {
        ...
        if (!isLongPress) {
          // Regular tap on mobile - open detail modal
          const node = event.target;
    ...
    const applyImprovedLayout = (
            ...
            const positionBranch = (
              ...
              const angleStep =
                branchChildren.length > 1
                  ? spreadAngle / (branchChildren.length - 1)
                  : 0;
        ...
        originalNodes.forEach((rootNode, index) => {
          ...
          if (sectorChildren.length > 0) {
            ...
            const childAngleStep =
              sectorChildren.length > 1
                ? sectorSpread / (sectorChildren.length - 1)
                : 0;
            ...
            sectorChildren.forEach((childId, childIndex) => {
              ...
              const childAngle =
                sectorChildren.length === 1
                  ? rootAngle
                  : sectorStartAngle + childIndex * childAngleStep;
              ...
              const childValidPos = findValidPosition(
                targetX,
                targetY,
                childId
              );
              ...
              if (grandchildren.length > 0) {
                ...
                const grandAngleStep =
                  grandchildren.length > 1
                    ? grandSpread / (grandchildren.length - 1)
                    : 0;
                ...
                grandchildren.forEach((grandId, grandIndex) => {
                  ...
                  const grandAngle =
                    grandchildren.length === 1
                      ? childAngle
                      : grandStartAngle + grandIndex * grandAngleStep;
                  const targetGrandX =
                    childValidPos.x + grandDistance * Math.cos(grandAngle);
                  const targetGrandY =
                    childValidPos.y + grandDistance * Math.sin(grandAngle);
                  const grandValidPos = findValidPosition(
                    targetGrandX,
                    targetGrandY,
                    grandId
                  );
      ...
      if (unpositioned.length > 0) {
        ...
        const orphanAngleStep =
          (2 * Math.PI) / Math.max(unpositioned.length, 1);
    ...
    const handleGenerateNodes = () => {
      onGenerateKeywords(
        generateKeywordsDialog.nodeName,
        generateKeywordsDialog.nodeCount,
        generateKeywordsDialog.isAutomatic
      );
      setGenerateKeywordsDialog({
        open: false,
        nodeName: "",
      ...
      <div className="relative w-full h-full bg-gradient-to-br from-slate-900 to-slate-800">
        ...
        {contextMenu.visible && (
          ...
          >
            ...
            <div
              className="flex items-center space-x-2 px-3 py-3 sm:py-2 text-sm text-slate-50 hover:bg-blue-600/10 hover:text-blue-400 rounded cursor-pointer transition-colors touch-manipulation"
              onClick={() => {
                handleGoogleSearch(contextMenu.nodeName, contextMenu.nodeId);
                hideContextMenu();
              }}
            >
              <WandSparkles className="w-4 h-4 flex-shrink-0" />
        ...
        <EditNodeModal
          ...
          onDescriptionChange={(description) =>
            setEditDialog((prev) => ({ ...prev, description }))
          }
        ...
        <AddSubNodeModal
          ...
          onOpenChange={(open) =>
            setAddSubNodeDialog((prev) => ({ ...prev, open }))
          }
          onNameChange={(name) =>
            setAddSubNodeDialog((prev) => ({ ...prev, name }))
          }
          onDescriptionChange={(description) =>
            setAddSubNodeDialog((prev) => ({ ...prev, description }))
          }
        ...
        <GenerateKeywordsModal
          ...
          onOpenChange={(open) =>
            setGenerateKeywordsDialog((prev) => ({ ...prev, open }))
          }
          onNodeCountChange={(nodeCount) =>
            setGenerateKeywordsDialog((prev) => ({ ...prev, nodeCount }))
          }
          onAutomaticChange={(isAutomatic) =>
            setGenerateKeywordsDialog((prev) => ({ ...prev, isAutomatic }))
          }
        ...
        <GoogleSearchConfirmModal
          open={googleSearchConfirmDialog.open}
          searchTerm={googleSearchConfirmDialog.searchTerm}
          initialSystemInstruction={googleSearchConfirmDialog.systemInstruction}
          isLoading={googleSearchMutation.isPending}
          onOpenChange={(open) =>
            setGoogleSearchConfirmDialog((prev) => ({ ...prev, open }))
          }
          onConfirm={handleConfirmedGoogleSearch}
          onCancel={() =>
            setGoogleSearchConfirmDialog((prev) => ({ ...prev, open: false }))
          }
        />
        ...
        <NodeDetailModal
          ...
          onOpenChange={(open) =>
            setNodeDetailDialog((prev) => ({ ...prev, open }))
          }
          onClose={() =>
