...
export default function TextChunker({ text }: { text: string }) {
  ...
  const handleChunkClick = (chunkId: string, event: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (document.caretRangeFromPoint) {
      ...
      const caretPosition = (document as DocumentWithCaretMethods).caretPositionFromPoint!(x + rect.left, y + rect.top)
    ...
    const renumberedChunks = newChunks.map((c, index) => ({
      ...c,
      name: c.name.includes(" - Part") ? `Chunk ${index + 1}` : `Chunk ${index + 1}`,
    }))

    setChunks(renumberedChunks)
  }

  const updateChunk = (chunkId: string, field: "name" | "description", value: string) => {
    setChunks(chunks.map((chunk) => (chunk.id === chunkId ? { ...chunk, [field]: value } : chunk)))
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm text-slate-300">
        Click anywhere in any chunk to split it into two parts. Each chunk can be named and described. 
        Use Ctrl+Z (or Cmd+Z on Mac) to undo the last split.
      </p>
      ...
      <div className="space-y-4">
        ...
        {chunks.map((chunk, index) => (
          <Card key={chunk.id} className="bg-slate-800 border-slate-600 overflow-hidden">
            <CardHeader className="pb-3">
              <CardTitle className="text-base flex items-center gap-2 text-slate-100">
                <div
                  className="w-3 h-3 rounded"
                  style={{ backgroundColor: `hsl(${(index * 137.5) % 360}, 60%, 60%)` }}
                />
                {chunk.name}
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid lg:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <Label className="text-sm font-medium text-slate-300">Text Content (Click to Split)</Label>
                  <div
                  ...
                  <p className="text-xs text-slate-400">
                    {chunk.text.length} characters • Click anywhere in the text above to split this chunk
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  <div>
                    <Label htmlFor={`name-${chunk.id}`} className="text-sm text-slate-300">Name/Title</Label>
                    <Input
                      id={`name-${chunk.id}`}
                      value={chunk.name}
                      onChange={(e) => updateChunk(chunk.id, "name", e.target.value)}
                  ...
                  <div>
                    <Label htmlFor={`desc-${chunk.id}`} className="text-sm text-slate-300">Description</Label>
                    <Textarea
                      id={`desc-${chunk.id}`}
