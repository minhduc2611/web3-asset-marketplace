...
const getAll = async (authorId: string): Promise<PostgrestSingleResponse<BrainLogTypesWithItems>> => {
  let response = await selectBrainLogTypesWithItem.eq('author_id', authorId);
  return response;
};
...
const insert = async (data: Pick<BrainLogModel, 'content' | 'brain_log_type_id' | 'author_id'>) => {
